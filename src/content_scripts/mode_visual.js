...
class Movement {
  static initClass() {
    this.prototype.opposite = {forward: backward, backward: forward}; // The existing range selection is backwards.
  
    // Test whether the character following the focus is a word character (and leave the selection unchanged).
    this.prototype.nextCharacterIsWordCharacter = (function() {
      let regexp = null;
      return function() {
  ...
  runMovement(...args) {
    ...
    const [ direction, granularity ] =
      Array.from((typeof(args[0]) === "string") && (args.length === 1) ?
        args[0].trim().split(/\s+/)
      :
    ...
    if ((granularity === vimword) && (direction === forward)) {
      while (this.nextCharacterIsWordCharacter()) {
        if (!this.runMovements([ forward, character ])) { return; }
      }
  ...
  hashSelection() {
    ...
    return [ this.selection.toString().length, range.anchorOffset, range.focusOffset, this.selection.extentOffset,
      this.selection.baseOffset ].join("/");
  }

  // Call a function; return true if the selection changed, false otherwise.
  selectionChanged(func) {
    const before = this.hashSelection(); func(); return this.hashSelection() !== before;
  }

  // Run a sequence of movements, stopping if a movement fails to change the selection.
  runMovements(...movements) {
    for (var movement of Array.from(movements)) {
      if (!this.selectionChanged(() => this.runMovement(movement))) { return false; }
    }
    return true;
  }
  ...
  reverseSelection() {
    ...
    if (element && DomUtils.isEditable(element) && !element.isContentEditable) {
      ...
      const { length } = this.selection.toString();
      this.collapseSelectionToFocus();
      return __range__(0, length, false).map((i) => this.runMovement(this.opposite[direction], character));
    } else {
      // Normal method.
  ...
  getDirection() {
    ...
    for (let direction of [ forward, backward ]) {
      ...
      if (change = this.extendByOneCharacter(direction)) {
        ...
        if (0 < change) { return direction; } else { return this.opposite[direction]; }
      }
    }
    return forward;
  }
  ...
  selectLexicalEntity(entity, count) {
    ...
    this.collapseSelectionToFocus();
    if (entity === word) { this.runMovement([ forward, character ]); } // This makes word movements a bit more vim-like.
    this.runMovement([ backward, entity ]);
    this.collapseSelectionToFocus();
    return (() => {
      const result = [];
      for (let i = 0, end = count; i < end; i++) {
        result.push(this.runMovement([ forward, entity ]));
      }
      return result;
    })();
  }
...
class VisualMode extends KeyHandlerMode {
  ...
  constructor(options) {
    ...
    for (let keys of Object.keys(this.movements || {})) {
      ...
      if ("function" === typeof movement) { movement = movement.bind(this); }
      if (keys.length === 1) {
        keyMapping[keys] = {command: movement};
      } else { // keys.length == 2
        if (keyMapping[keys[0]] == null) { keyMapping[keys[0]] = {}; }
    ...
    extend(keyMapping, {
      "B": keyMapping.b,
      "W": keyMapping.w,
      "<c-e>": { command(count) { return Scroller.scrollBy("y", count * Settings.get("scrollStepSize"), 1, false); }
    },
      "<c-y>": { command(count) { return Scroller.scrollBy("y", -count * Settings.get("scrollStepSize"), 1, false); }
    }
    }
    );
    ...
    this.onExit((event = null) => {
      ...
      } else if (((event != null ? event.type : undefined) === "keydown") && KeyboardUtils.isEscape(event) && (this.name !== "caret")) {
        this.movement.collapseSelectionToFocus();
      } else {
        this.movement.collapseSelectionToAnchor();
      }
      ...
      if (document.activeElement && DomUtils.isEditable(document.activeElement)) {
        if ((event != null ? event.type : undefined) !== "click") { return document.activeElement.blur(); }
      }
    });
    ...
    if (this.name !== "caret") {
      let needle;
      if ((needle = DomUtils.getSelectionType(this.selection), [ "Caret", "Range" ].includes(needle))) {
        ...
        if (window.vimiumDomTestsAreRunning) {
          ...
          if (!selectionRect) { selectionRect = {top: 0, bottom: 0, left: 0, right: 0, width: 0, height: 0}; }
        }
  ...
  commandHandler({command: {command}, count}) {
    switch (typeof command) {
      case "string":
        for (let i = 0, end = count; i < end; i++) { this.movement.runMovement(command); }
        break;
      case "function":
        command(count);
        break;
  ...
  find(count, backwards) {
    ...
    for (let i = 0, end = count; i < end; i++) {
      if (!FindMode.execute(null, {colorSelection: false, backwards})) {
    ...
    if ((this.name === "caret") && (0 < this.selection.toString().length)) { return new VisualMode; }
  }
  ...
  yank(args) {
    ...
    if (15 < this.yankedText.length) { message = message.slice(0, 12) + "..."; }
...
class VisualLineMode extends VisualMode {
  ...
  commandHandler(...args) {
    super.commandHandler(...Array.from(args || []));
    if (this.modeIsActive) { return this.extendSelection(); }
  }
...
class CaretMode extends VisualMode {
  ...
  commandHandler(...args) {
    this.movement.collapseSelectionToAnchor();
    super.commandHandler(...Array.from(args || []));
    if (this.modeIsActive) { return this.movement.extendByOneCharacter(forward); }
  }
