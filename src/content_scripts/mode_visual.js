...
class Movement {
  ...
  runMovement(...args) {
    ...
    const [direction, granularity] = Array.from((typeof (args[0]) === 'string') && (args.length === 1)
      ? args[0].trim().split(/\s+/)
  ...
  getDirection() {
    ...
    for (const direction of [forward, backward]) {
      ...
      if (change = this.extendByOneCharacter(direction)) {
        ...
        if (change > 0) { return direction; } return this.opposite[direction];
      }
    }
    return forward;
  }
...
class VisualMode extends KeyHandlerMode {
  ...
  constructor(options) {
    ...
    for (const keys of Object.keys(this.movements || {})) {
      ...
      if (typeof movement === 'function') { movement = movement.bind(this); }
      if (keys.length === 1) {
  ...
  find(count, backwards) {
    ...
    if ((this.name === 'caret') && (this.selection.toString().length > 0)) { return new VisualMode(); }
  }
  ...
  yank(args) {
    ...
    if (this.yankedText.length > 15) { message = `${message.slice(0, 12)}...`; }
...
class CaretMode extends VisualMode {
  ...
  establishInitialSelectionAnchor() {
    ...
    while ((node = nodes.nextNode())) {
      ...
      if ((node.nodeType === 3) && (node.data.trim().length >= 50)) {
