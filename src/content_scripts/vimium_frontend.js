...
const installListener = (element, event, callback) =>
  element.addEventListener(event, forTrusted(function() {
    if (typeof extend === 'undefined' || extend === null) { root.extend(window, root); } // See #2800.
    if (isEnabledForUrl) { return callback.apply(this, arguments); } else { return true; }
  }), true)
;
...
const flashFrame = (function() {
  ...
  return function() {
    if (highlightedFrameElement == null) { highlightedFrameElement = (function() {
...
if (root.HelpDialog == null) { root.HelpDialog = {
  helpUI: null,
  ...
  toggle(request) {
    DomUtils.documentComplete(() => {
      return this.helpUI != null ? this.helpUI : (this.helpUI = new UIComponent("pages/help_dialog.html", "vimiumHelpDialogFrame", function() {}));
    });
