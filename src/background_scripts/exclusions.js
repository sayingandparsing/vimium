...
var Exclusions = {
  ...
  getRule(url, rules) {
    ...
    const matchingRules = ((() => {
      const result = [];
      for (rule of Array.from(rules)) {         if (rule.pattern && (0 <= url.search(RegexpCache.get(rule.pattern)))) {
          result.push(rule);
        }
      }
      return result;
    ...
    const passKeys = ((() => {
      const result1 = [];
      for (rule of Array.from(matchingRules)) {         result1.push(rule.passKeys.split(/\s+/).join(""));
      }
      return result1;
    })()).join("");
