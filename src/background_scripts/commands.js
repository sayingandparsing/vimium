...
const Commands = {
  ...
  init() {
    for (let command of Object.keys(commandDescriptions || {})) {
      ...
      this.availableCommands[command] = extend((options != null ? options : {}), {description});
    }
  ...
  loadKeyMappings(customKeyMappings) {
    ...
    const configLines = ((() => {
      const result = [];
      for (key of Object.keys(defaultKeyMappings || {})) {
        command = defaultKeyMappings[key];
        result.push(`map ${key} ${command}`);
      }
      return result;
    })());
    configLines.push(...Array.from(BgUtils.parseLines(customKeyMappings) || []));
    const seen = {};
    let unmapAll = false;
    for (let line of Array.from(configLines.reverse())) {
      const tokens = line.split(/\s+/);
      switch (tokens[0].toLowerCase()) {
        case "map":
          if ((3 <= tokens.length) && !unmapAll) {
            var _, optionList, registryEntry;
            [_, key, command, ...optionList] = Array.from(tokens);
    ...
    // mode.
    return Settings.set("passNextKeyKeys",
      ((() => {
      const result1 = [];
      for (key of Object.keys(this.keyToCommandRegistry || {})) {
  ...
  parseKeySequence: (function() {
    ...
    return function(key) {
      ...
      // Parse "<c-a>bcd" as "<c-a>" and "bcd".
        ...
        let array = RegExp.$1.split("-"),
          adjustedLength = Math.max(array.length, 1),
          modifiers = array.slice(0, adjustedLength - 1),
          keyChar = array[adjustedLength - 1];
        if (keyChar.length !== 1) { keyChar = keyChar.toLowerCase(); }
        modifiers = ((() => {
          const result = [];
          for (modifier of Array.from(modifiers)) {             result.push(modifier.toLowerCase());
          }
          return result;
        })());
        modifiers.sort();
        return [`<${[...Array.from(modifiers), keyChar].join('-')}>`, ...Array.from(this.parseKeySequence(RegExp.$2))];
      } else {
        return [key[0], ...Array.from(this.parseKeySequence(key.slice(1)))];
      }
    };
  })(),
  ...
  parseCommandOptions(command, optionList) {
    ...
    if ("count" in options) {
      ...
      if (isNaN(options.count) || this.availableCommands[command].noRepeat) { delete options.count; }
    }
  ...
  installKeyStateMapping() {
    ...
    for (let keys of Object.keys(this.keyToCommandRegistry || {})) {
      ...
      for (let index = 0; index < registryEntry.keySequence.length; index++) {
        ...
        if (currentMapping[key] != null ? currentMapping[key].command : undefined) {
          ...
          for (let prop of ["keySequence", "description"]) { delete currentMapping[key][prop]; }
        }
      }
    }
    ...
    // default).
    return chrome.storage.local.set({useVimLikeEscape: !("<c-[>" in keyStateMapping)});
  },
    ...
    for (let group of Object.keys(this.commandGroups || {})) {
      ...
      for (let command of Array.from(commands)) {
        commandGroups[group].push({
          ...
          advanced: Array.from(this.advancedCommands).includes(command)
        });
      }
    }
    return chrome.storage.local.set({helpPageData: commandGroups});
  },
...
var commandDescriptions = {
  ...
  removeTab: ["Close current tab", { background: true, repeatLimit: (chrome.session != null ? chrome.session.MAX_SESSION_RESULTS : undefined) != null ? (chrome.session != null ? chrome.session.MAX_SESSION_RESULTS : undefined) : 25 }],
