...
const Commands = {
  ...
  loadKeyMappings(customKeyMappings) {
    ...
    const seen = {};
    let unmapAll = false;
    for (const line of Array.from(configLines.reverse())) {
      const tokens = line.split(/\s+/);
      switch (tokens[0].toLowerCase()) {
        case 'map':
          if ((tokens.length >= 3) && !unmapAll) {
            var _; var optionList; var
              registryEntry;
    ...
    return Settings.set('passNextKeyKeys',
      ((() => {
        ...
        for (key of Object.keys(this.keyToCommandRegistry || {})) {
          if ((this.keyToCommandRegistry[key].command === 'passNextKey') && (key.length > 1)) {
            result1.push(key);
          }
        }
        return result1;
  ...
  parseKeySequence: (function () {
    ...
    return function (key) {
      ...
      // Parse "<c-a>bcd" as "<c-a>" and "bcd".
      } if (key.search(specialKeyRegexp) === 0) {
        ...
        modifiers = ((() => {
          ...
          for (modifier of Array.from(modifiers)) {
            result.push(modifier.toLowerCase());
          }
          return result;
        })());
        modifiers.sort();
    ...
    for (const keys of Object.keys(this.keyToCommandRegistry || {})) {
      ...
      for (let index = 0; index < registryEntry.keySequence.length; index++) {
        ...
        if (currentMapping[key] != null ? currentMapping[key].command : undefined) {
          ...
          for (const prop of ['keySequence', 'description']) { delete currentMapping[key][prop]; }
        }
      }
    }
