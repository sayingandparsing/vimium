...
const Settings = {
    ...
    return chrome.storage.sync.get(null, () => {
      ...
      return chrome.storage.local.get(null, (localItems) => {
        ...
        return this.storage.get(null, (syncedItems) => {
          ...
          return this.runOnLoadedCallbacks();
        });
      });
    });
  },
  ...
  use(key, callback) {
    ...
    return this.onLoaded(() => callback(this.get(key)));
  },

  // For settings which require action when their value changes, add hooks to this object.
  postUpdateHooks: {},
  performPostUpdateHook(key, value) { return (typeof this.postUpdateHooks[key] === 'function' ? this.postUpdateHooks[key](value) : undefined); },
  ...
  nuke(key) {
    ...
    chrome.storage.local.remove(key);
    return (chrome.storage.sync != null ? chrome.storage.sync.remove(key) : undefined);
  },
