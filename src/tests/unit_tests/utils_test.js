...
context("isUrl",
  should("accept valid URLs", function() {
    ...
    // Internal URLs.
    return assert.isTrue(Utils.isUrl("moz-extension://c66906b4-3785-4a60-97bc-094a6366017e/pages/options.html"));
  }),
...
context("convertToUrl",
  should("detect and clean up valid URLs", function() {
...
context("extractQuery",
  should("extract queries from search URLs", function() {
  ...
  should("extract not queries from incorrect search URLs", () => assert.isFalse(Utils.extractQuery("https://www.google.ie/search?q=%s&foo=bar", "https://www.google.ie/search?q=bbc+sport")))
);

context("hasChromePrefix",
  should("detect chrome prefixes of URLs", function() {
...
context("hasJavascriptPrefix",
  should("detect javascript: URLs", function() {
...
context("decodeURIByParts",
  should("decode javascript: URLs", function() {
...
context("Function currying",
  should("Curry correctly", function() {
    const foo = (a, b) => `${a},${b}`;
...
context("compare versions",
  should("compare correctly", function() {
...
context("makeIdempotent",
  setup(function() {
    this.count = 0;
    return this.func = Utils.makeIdempotent(n => { if (n == null) { n = 1; } return this.count += n; });
  }),
...
context("escapeRegexSpecialCharacters",
  should("escape regexp special characters", function() {
    const str = "-[]/{}()*+?.^$|";
...
context("extractQuery",
  should("extract the query terms from a URL", function() {
